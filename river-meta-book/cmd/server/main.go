...
func main() {
	...
	api := apiclient.New(cfg.RiverAPIURL, cfg.RiverAPIUsername, cfg.RiverAPIPassword, "river-meta-book")
	...
	go func() {
		...
		mux.HandleFunc("POST /refresh/{id}", func(w http.ResponseWriter, r *http.Request) {
			id := r.PathValue("id")
			if err := proc.RefreshByID(id); err != nil {
				log.Printf("ERROR refresh audiobook %s: %v", id, err)
