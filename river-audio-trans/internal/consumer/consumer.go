...
type MediaDiscoveredEvent struct {
	...
	// scanner has already registered the media records against the
	// source files.
...
func New(url, exchange string) (*Consumer, error) {
	...
	q, err := ch.QueueDeclare("river.audio.trans", true, false, false, false, nil)
	if err != nil {
		ch.Close()
		conn.Close()
		return nil, fmt.Errorf("declare queue: %w", err)
	}
	for _, key := range []string{"media.discovered.music", "media.discovered.audiobook"} {
...
// Consume calls handler for each message, ACKing on success and NACKing (no requeue) on error.
