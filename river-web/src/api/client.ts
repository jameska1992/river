...
export class RiverClient {
  ...
  async getScannerState(): Promise<{
    ...
    return this.request('GET', '/admin/scanner-state')
  }

  async forgetScannerState(req: { paths?: string[]; prefixes?: string[]; shows?: string[] }): Promise<void> {
    await this.request('POST', '/admin/scanner-state/forget', req)
  }
  ...
  // deleteEpisode removes one episode row. Set deleteFiles=true to also
  // remove the transcoded copy + the original source on disk. The
  // scanner forgets the season directory's hash either way so the next
  // scan re-publishes that season (recreating the row when the source
  // is still on disk).
  async deleteEpisode(showId: string, seasonId: string, episodeId: string, deleteFiles = false): Promise<void> {
    const qs = deleteFiles ? '?delete_files=true' : ''
  ...
  // getCalendar returns the combined Radarr + Sonarr calendar for [start, end]
  // (dates as YYYY-MM-DD), sorted by date ascending.
  async getCalendar(start: string, end: string): Promise<import('./types').CalendarItem[]> {
    return this.request('GET', `/request/calendar?start=${encodeURIComponent(start)}&end=${encodeURIComponent(end)}`)
  }
