...
// topic exchange. This service emits media.enriched.* after a successful metadata
// update; river-events joins that with transcode into media.ready.*.
//
// The payload mirrors the versioned contract in the river-events service — keep
// the two in sync when the schema changes (SchemaVersion is bumped there).
package lifecycle
