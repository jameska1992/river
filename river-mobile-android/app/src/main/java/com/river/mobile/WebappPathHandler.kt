...
/**
 * Serves the bundled river-mobile Vite build from `assets/webapp/`.
 *
 * If the requested path doesn't map to a real file *and* looks like an SPA
 * route (no file extension in the last segment, or empty), we fall back to
 * `index.html` so React Router's BrowserRouter can handle deep links like
 * `/albums/abc-123` on cold-start or reload. Requests that clearly want a
 * static asset (`foo.js`, `bar.png`) return 404 instead, so missing-file bugs
 * surface loudly rather than silently serving the HTML shell.
 */
