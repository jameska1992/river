...
// Represents "the next episode of a show you're partway through" —
// distinct from Continue Watching, which is "resume mid-episode."
export interface NextUpItem {
  ...
  season_number: number
  episode_number: number
  // Timestamp of the previously-completed episode that anchored this
  // suggestion. Used only for sort order client-side.
  updated_at: string
}

// SimilarItem is the trimmed shape returned by the /*/similar
// endpoints — one entry per "more like this" card in the recommended
// carousel on movie / show / audiobook detail pages.
...
// Combined upcoming-release entry from Radarr (movies) and Sonarr (episodes).
export interface CalendarItem {
  ...
  releaseType?: 'digital' | 'physical' | 'cinema'
  date: string
